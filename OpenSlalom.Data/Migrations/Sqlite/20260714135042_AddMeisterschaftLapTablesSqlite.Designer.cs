// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using OpenSlalom.Data;

#nullable disable

namespace OpenSlalom.Data.Migrations.Sqlite
{
    [DbContext(typeof(LocalOpenSlalomDbContext))]
    [Migration("20260714135042_AddMeisterschaftLapTablesSqlite")]
    partial class AddMeisterschaftLapTablesSqlite
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder.HasAnnotation("ProductVersion", "8.0.2");

            modelBuilder.Entity("OpenSlalom.Data.Entities.Disziplin", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("TEXT")
                        .HasColumnName("disziplin");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<double>("ZeitstrafePylonenfehler")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("REAL")
                        .HasDefaultValue(0.0)
                        .HasColumnName("pf");

                    b.Property<double>("ZeitstrafeTorfehler")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("REAL")
                        .HasDefaultValue(0.0)
                        .HasColumnName("tf");

                    b.HasKey("Id");

                    b.ToTable("disziplin", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.DisziplinAltersklasse", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<int?>("AlterBis")
                        .HasColumnType("INTEGER")
                        .HasColumnName("alter_bis");

                    b.Property<int>("AlterVon")
                        .HasColumnType("INTEGER")
                        .HasColumnName("alter_von");

                    b.Property<string>("Bezeichnung")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT")
                        .HasColumnName("bezeichnung");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<int>("DisziplinId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_disziplin");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.HasKey("Id");

                    b.HasIndex("DisziplinId");

                    b.ToTable("disziplin_altersklassen", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Fahrer", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<DateOnly?>("Geburtsdatum")
                        .HasColumnType("date")
                        .HasColumnName("geburtsdatum");

                    b.Property<string>("Geschlecht")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("TEXT")
                        .HasDefaultValue("")
                        .HasColumnName("geschlecht");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Nachname")
                        .HasMaxLength(100)
                        .HasColumnType("TEXT")
                        .HasColumnName("nachname");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("VereinId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_verein");

                    b.Property<string>("Vorname")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT")
                        .HasColumnName("vorname");

                    b.HasKey("Id");

                    b.HasIndex("VereinId");

                    b.ToTable("fahrer", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.FahrerImTraining", b =>
                {
                    b.Property<int>("TrainingId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_training");

                    b.Property<int>("FahrerId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_fahrer");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<int>("Reihenfolge")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(0)
                        .HasColumnName("reihenfolge");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.HasKey("TrainingId", "FahrerId");

                    b.HasIndex("FahrerId");

                    b.ToTable("fahrer_im_training", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.FahrerInDerMeisterschaft", b =>
                {
                    b.Property<int>("MeisterschaftId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_meisterschaft");

                    b.Property<int>("FahrerId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_fahrer");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<int>("Reihenfolge")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(0)
                        .HasColumnName("reihenfolge");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.HasKey("MeisterschaftId", "FahrerId");

                    b.HasIndex("FahrerId");

                    b.ToTable("fahrer_inder_meisterschaft", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Kart", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<string>("Chassis")
                        .HasMaxLength(100)
                        .HasColumnType("TEXT")
                        .HasColumnName("Chassis");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<int>("DisziplinId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_disziplin");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Motor")
                        .HasMaxLength(100)
                        .HasColumnType("TEXT")
                        .HasColumnName("Motor");

                    b.Property<string>("Name")
                        .HasMaxLength(100)
                        .HasColumnType("TEXT")
                        .HasColumnName("Name");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("VereinId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_verein");

                    b.HasKey("Id");

                    b.HasIndex("DisziplinId");

                    b.HasIndex("VereinId");

                    b.ToTable("karts", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Meisterschaft", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<bool>("AktivAusgerichtet")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("aktiv_ausgerichtet");

                    b.Property<string>("Beschreibung")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("TEXT")
                        .HasColumnName("beschreibung");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<int>("DisziplinId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_disziplin");

                    b.Property<int>("GastgeberId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_gastgeber");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<bool>("MeisterschaftAbgeschlossen")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("meisterschaft_abgeschlossen");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT")
                        .HasColumnName("name");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("WetterId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_wetter");

                    b.Property<DateOnly>("Zeitpunkt")
                        .HasColumnType("date")
                        .HasColumnName("zeitpunkt");

                    b.HasKey("Id");

                    b.HasIndex("DisziplinId");

                    b.HasIndex("GastgeberId");

                    b.HasIndex("WetterId");

                    b.ToTable("meisterschaften", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Mrunde", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<int?>("MstintId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_mstint");

                    b.Property<int?>("Pf")
                        .HasColumnType("INTEGER")
                        .HasColumnName("pf");

                    b.Property<int?>("Runde")
                        .HasColumnType("INTEGER")
                        .HasColumnName("runde");

                    b.Property<double?>("Rundenzeit")
                        .HasColumnType("REAL")
                        .HasColumnName("rundenzeit");

                    b.Property<int?>("Tf")
                        .HasColumnType("INTEGER")
                        .HasColumnName("tf");

                    b.Property<bool>("Ungueltig")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("ungueltig");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.HasKey("Id");

                    b.HasIndex("MstintId");

                    b.ToTable("mrunden", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Mstint", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<string>("AltersklasseSnapshot")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT")
                        .HasDefaultValue("")
                        .HasColumnName("altersklasse_snapshot");

                    b.Property<DateTime>("Datum")
                        .HasColumnType("datetime")
                        .HasColumnName("datum");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<int>("FahrerId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_fahrer");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<int?>("KartId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_kart");

                    b.Property<int>("MeisterschaftId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_meisterschaft");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.HasKey("Id");

                    b.HasIndex("FahrerId");

                    b.HasIndex("KartId");

                    b.HasIndex("MeisterschaftId");

                    b.ToTable("mstints", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.SyncState", b =>
                {
                    b.Property<string>("Id")
                        .HasMaxLength(50)
                        .HasColumnType("TEXT")
                        .HasColumnName("id");

                    b.Property<DateTime>("LastSyncUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("last_sync_utc");

                    b.HasKey("Id");

                    b.ToTable("sync_state", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Training", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<string>("Beschreibung")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("TEXT")
                        .HasColumnName("beschreibung");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<int>("DisziplinId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_disziplin");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT")
                        .HasColumnName("name");

                    b.Property<bool>("TrainingAbgeschlossen")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("training_abgeschlossen");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("VereinId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_verein");

                    b.Property<int>("WetterId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_wetter");

                    b.Property<DateOnly>("Zeitpunkt")
                        .HasColumnType("date")
                        .HasColumnName("zeitpunkt");

                    b.HasKey("Id");

                    b.HasIndex("DisziplinId");

                    b.HasIndex("VereinId");

                    b.HasIndex("WetterId");

                    b.ToTable("training", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Trunde", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<int?>("Pf")
                        .HasColumnType("INTEGER")
                        .HasColumnName("pf");

                    b.Property<int?>("Runde")
                        .HasColumnType("INTEGER")
                        .HasColumnName("runde");

                    b.Property<double?>("Rundenzeit")
                        .HasColumnType("REAL")
                        .HasColumnName("rundenzeit");

                    b.Property<int?>("Tf")
                        .HasColumnType("INTEGER")
                        .HasColumnName("tf");

                    b.Property<int?>("TstintId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_tstint");

                    b.Property<bool>("Ungueltig")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("ungueltig");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.HasKey("Id");

                    b.HasIndex("TstintId");

                    b.ToTable("trunden", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Tstint", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<string>("AltersklasseSnapshot")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT")
                        .HasDefaultValue("")
                        .HasColumnName("altersklasse_snapshot");

                    b.Property<DateTime>("Datum")
                        .HasColumnType("datetime")
                        .HasColumnName("datum");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<int>("FahrerId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_fahrer");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<int?>("KartId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_kart");

                    b.Property<int>("TrainingId")
                        .HasColumnType("INTEGER")
                        .HasColumnName("fk_id_training");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.HasKey("Id");

                    b.HasIndex("FahrerId");

                    b.HasIndex("KartId");

                    b.HasIndex("TrainingId");

                    b.ToTable("tstints", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Verein", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("MitgliedsNummer")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("TEXT")
                        .HasDefaultValue("")
                        .HasColumnName("mitglieds_nummer");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("Vereinsname")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT")
                        .HasDefaultValue("")
                        .HasColumnName("vereinsname");

                    b.HasKey("Id");

                    b.ToTable("vereine", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Wetter", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasColumnName("id");

                    b.Property<string>("Bezeichnung")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("TEXT")
                        .HasColumnName("wetter");

                    b.Property<DateTime?>("DeletedAtUtc")
                        .HasColumnType("datetime")
                        .HasColumnName("deleted_at_utc");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<DateTime>("UpdatedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at_utc")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.HasKey("Id");

                    b.ToTable("wetter", (string)null);
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.DisziplinAltersklasse", b =>
                {
                    b.HasOne("OpenSlalom.Data.Entities.Disziplin", "Disziplin")
                        .WithMany("Altersklassen")
                        .HasForeignKey("DisziplinId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_disziplin_altersklassen_disziplin");

                    b.Navigation("Disziplin");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Fahrer", b =>
                {
                    b.HasOne("OpenSlalom.Data.Entities.Verein", "Verein")
                        .WithMany("Fahrer")
                        .HasForeignKey("VereinId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_fahrer_vereine");

                    b.Navigation("Verein");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.FahrerImTraining", b =>
                {
                    b.HasOne("OpenSlalom.Data.Entities.Fahrer", "Fahrer")
                        .WithMany("FahrerImTrainings")
                        .HasForeignKey("FahrerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_fahrer_im_training_fahrer");

                    b.HasOne("OpenSlalom.Data.Entities.Training", "Training")
                        .WithMany("FahrerImTrainings")
                        .HasForeignKey("TrainingId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_fahrer_im_training_training");

                    b.Navigation("Fahrer");

                    b.Navigation("Training");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.FahrerInDerMeisterschaft", b =>
                {
                    b.HasOne("OpenSlalom.Data.Entities.Fahrer", "Fahrer")
                        .WithMany("FahrerInDerMeisterschaften")
                        .HasForeignKey("FahrerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_fahrer_inder_meisterschaft_fahrer");

                    b.HasOne("OpenSlalom.Data.Entities.Meisterschaft", "Meisterschaft")
                        .WithMany("FahrerInDerMeisterschaften")
                        .HasForeignKey("MeisterschaftId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_fahrer_inder_meisterschaft_meisterschaften");

                    b.Navigation("Fahrer");

                    b.Navigation("Meisterschaft");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Kart", b =>
                {
                    b.HasOne("OpenSlalom.Data.Entities.Disziplin", "Disziplin")
                        .WithMany("Karts")
                        .HasForeignKey("DisziplinId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_karts_disziplin");

                    b.HasOne("OpenSlalom.Data.Entities.Verein", "Verein")
                        .WithMany("Karts")
                        .HasForeignKey("VereinId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_karts_vereine");

                    b.Navigation("Disziplin");

                    b.Navigation("Verein");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Meisterschaft", b =>
                {
                    b.HasOne("OpenSlalom.Data.Entities.Disziplin", "Disziplin")
                        .WithMany("Meisterschaften")
                        .HasForeignKey("DisziplinId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_meisterschaften_disziplin");

                    b.HasOne("OpenSlalom.Data.Entities.Verein", "Gastgeber")
                        .WithMany("Meisterschaften")
                        .HasForeignKey("GastgeberId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_meisterschaften_vereine");

                    b.HasOne("OpenSlalom.Data.Entities.Wetter", "Wetter")
                        .WithMany("Meisterschaften")
                        .HasForeignKey("WetterId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_meisterschaften_wetter");

                    b.Navigation("Disziplin");

                    b.Navigation("Gastgeber");

                    b.Navigation("Wetter");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Mrunde", b =>
                {
                    b.HasOne("OpenSlalom.Data.Entities.Mstint", "Mstint")
                        .WithMany("Mrunden")
                        .HasForeignKey("MstintId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_mrunden_mstints");

                    b.Navigation("Mstint");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Mstint", b =>
                {
                    b.HasOne("OpenSlalom.Data.Entities.Fahrer", "Fahrer")
                        .WithMany("Mstints")
                        .HasForeignKey("FahrerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_mstints_fahrer");

                    b.HasOne("OpenSlalom.Data.Entities.Kart", "Kart")
                        .WithMany()
                        .HasForeignKey("KartId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_mstints_karts");

                    b.HasOne("OpenSlalom.Data.Entities.Meisterschaft", "Meisterschaft")
                        .WithMany("Mstints")
                        .HasForeignKey("MeisterschaftId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_mstints_meisterschaften");

                    b.Navigation("Fahrer");

                    b.Navigation("Kart");

                    b.Navigation("Meisterschaft");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Training", b =>
                {
                    b.HasOne("OpenSlalom.Data.Entities.Disziplin", "Disziplin")
                        .WithMany("Trainings")
                        .HasForeignKey("DisziplinId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_training_disziplin");

                    b.HasOne("OpenSlalom.Data.Entities.Verein", "Verein")
                        .WithMany("Trainings")
                        .HasForeignKey("VereinId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_training_vereine");

                    b.HasOne("OpenSlalom.Data.Entities.Wetter", "Wetter")
                        .WithMany("Trainings")
                        .HasForeignKey("WetterId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_training_wetter");

                    b.Navigation("Disziplin");

                    b.Navigation("Verein");

                    b.Navigation("Wetter");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Trunde", b =>
                {
                    b.HasOne("OpenSlalom.Data.Entities.Tstint", "Tstint")
                        .WithMany("Trunden")
                        .HasForeignKey("TstintId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_trunden_tstints");

                    b.Navigation("Tstint");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Tstint", b =>
                {
                    b.HasOne("OpenSlalom.Data.Entities.Fahrer", "Fahrer")
                        .WithMany("Tstints")
                        .HasForeignKey("FahrerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_tstints_fahrer");

                    b.HasOne("OpenSlalom.Data.Entities.Kart", "Kart")
                        .WithMany()
                        .HasForeignKey("KartId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_tstints_karts");

                    b.HasOne("OpenSlalom.Data.Entities.Training", "Training")
                        .WithMany("Tstints")
                        .HasForeignKey("TrainingId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_tstints_training");

                    b.Navigation("Fahrer");

                    b.Navigation("Kart");

                    b.Navigation("Training");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Disziplin", b =>
                {
                    b.Navigation("Altersklassen");

                    b.Navigation("Karts");

                    b.Navigation("Meisterschaften");

                    b.Navigation("Trainings");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Fahrer", b =>
                {
                    b.Navigation("FahrerImTrainings");

                    b.Navigation("FahrerInDerMeisterschaften");

                    b.Navigation("Mstints");

                    b.Navigation("Tstints");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Meisterschaft", b =>
                {
                    b.Navigation("FahrerInDerMeisterschaften");

                    b.Navigation("Mstints");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Mstint", b =>
                {
                    b.Navigation("Mrunden");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Training", b =>
                {
                    b.Navigation("FahrerImTrainings");

                    b.Navigation("Tstints");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Tstint", b =>
                {
                    b.Navigation("Trunden");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Verein", b =>
                {
                    b.Navigation("Fahrer");

                    b.Navigation("Karts");

                    b.Navigation("Meisterschaften");

                    b.Navigation("Trainings");
                });

            modelBuilder.Entity("OpenSlalom.Data.Entities.Wetter", b =>
                {
                    b.Navigation("Meisterschaften");

                    b.Navigation("Trainings");
                });
#pragma warning restore 612, 618
        }
    }
}
